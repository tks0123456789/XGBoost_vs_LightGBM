...
for exp_dict in exp_lst:
    ...
    for n_train in exp_dict['n_train_s']:
        leaf_cnts = []
        loss_valid = []
        max_depth_lst = exp_dict['max_depth_s']
        for max_depth in max_depth_lst:
            fname_footer = "_n_%d_md_%d.csv" % (n_train, max_depth)
        ...
        dname = header[:-1] + ", n_train:%s" % str_n_train
        ...
        plt.savefig("img/" + header + "leafcnts_%s.png" % str_n_train)
        ...
        axes[max_depth_cnts - 1].set_xlabel("Boosting iterations")
        plt.savefig("img/" + header + metric + "_%s.png" % str_n_train)
